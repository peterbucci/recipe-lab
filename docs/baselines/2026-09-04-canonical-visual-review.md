...
The integration merge-readiness run exposed 64 reference mismatches, each
...
`frontend/baselines/{project}/{name}.png` paths. Only those 64 expected files
and their exact Git blob IDs in `EXPORT_POLICY` change. Artifact directories,
actual/diff filenames, reports, and private browser evidence remain excluded.
...
accessibility or behavioral acceptance evidence.
