...
interface RecipeDraftWorkspacePageProps {
  params: Promise<{ draftId: string }>;
}

export default async function RecipeDraftWorkspacePage({
  params,
}: RecipeDraftWorkspacePageProps) {
  const { draftId } = await params;
  if (!isRecipeVersionId(draftId)) notFound();

  const [ingredientUnits, durationUnits, temperatureUnits, actionTypes] =
    await Promise.all([
