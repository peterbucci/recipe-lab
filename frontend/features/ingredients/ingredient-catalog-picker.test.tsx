...
import { searchCatalogIngredients } from "./catalog/ingredient-catalog-api";
import { submitMissingIngredientRequest } from "./requests/ingredient-request-api";
import { deferred } from "../../tests/support/deferred";
import { IngredientCatalogPicker } from "./ingredient-catalog-picker";

type PickerRequest = NonNullable<
  Parameters<typeof IngredientCatalogPicker>[0]["requestValue"]
>;
...
vi.mock("./catalog/ingredient-catalog-api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./catalog/ingredient-catalog-api")>();
  return {
    ...actual,
...
vi.mock("./requests/ingredient-request-api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./requests/ingredient-request-api")>();
  return {
    ...actual,
...
function PickerHarness({
  ...
  disabled?: boolean;
  initialRequest?: PickerRequest | null;
