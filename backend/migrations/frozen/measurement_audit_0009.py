"""Frozen legacy-measure audit used only by migration 0009."""
...
class MeasurementAuditError(RuntimeError):
    """Raised when the historical schema cannot be audited safely."""
...
def _lookup_tokens(unit: FrozenMeasurementUnit) -> set[str]:
    values = {
        unit.key,
...
def _catalog_lookup(
    catalog: FrozenMeasurementCatalog,
) -> tuple[dict[str, FrozenMeasurementUnit], dict[str, tuple[str, ...]], str]:
...
def _classify_row(
    row: Mapping[str, Any],
    *,
    units_by_key: Mapping[str, FrozenMeasurementUnit],
...
def build_legacy_measurement_audit(connection: Connection) -> dict[str, Any]:
    """Return the immutable migration-0009 audit report without writing."""
