  ...
  for _ in {1..600}; do
    ...
    if ! kill -0 "$browser_pid" 2>/dev/null; then
      ...
      if [[ "$safe_location" =~ ^[1-9][0-9]*:[1-9][0-9]*$ ]]; then
        ...
        echo "::error file=frontend/e2e/release/community-release-gate.spec.ts,line=$failure_line,col=$failure_column::RCP-32 failed at this assertion; private diagnostics were withheld."
      fi
